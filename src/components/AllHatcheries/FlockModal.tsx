...
> = ({ closeForm, onSubmit, initialData = null, preselectedHatchery }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      dateOfPlacement: formData.dateOfPlacement,
  ...
  const breedOptions: OptionValues[] = [
    ...
    ...loadBreed
      ...
      .map((b) => ({
        option: `GRANDPARENT - ${b.name}`,
        value: b.id.toString(),
      })),
    ...loadBreed
      .filter((b) => b.generation === "BROILER")
      .map((b) => ({ option: `BROILER - ${b.name}`, value: b.id.toString() })),
    ...loadBreed
      .filter((b) => b.generation === "LAYER")
      .map((b) => ({ option: `LAYER - ${b.name}`, value: b.id.toString() })),
  ];
  ...
  const parentOptions: OptionValues[] = [
    ...
    ...loadBreed
      ...
      .map((b) => ({ option: `${b.name}`, value: b.id.toString() })),
  ];
          ...
          <div className="flex flex-col w-xs relative px-3">
            ...
            <div className="relative w-full">
              ...
              <input
                ...
                className="w-full border border-border rounded-lg px-9 py-2 text-base"
                value={searchQuery}
                onChange={(e) => {
