...
const HatcheryMember = ({ editUserId }: { editUserId?: string | null }) => {
  ...
  const [emptyRow, setEmptyRow] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setIsSearching(true);
      try {
        const res = await searchHatcheryMember(search.trim());
        const data = res.data;

        const processedData = data.slice(0, currentLimit);
                  ...
                  {(membersToShow as HatcheryMemberType[]).map(
                    (hatchery, i) => (
                      ...
                      >
                        ...
                        {/* Email */}
                        <td className="cell text-left max-w-xs truncate">
